...
/*router.post('/addEmployee', (req, res) => {
    const { employeeName, email, contactNo, username, password } = req.body;
  
    const insertQuery = 'INSERT INTO employees (employeeName, email, contactNo, username, password) VALUES (?, ?, ?, ?, ?)';
  
    db.query(insertQuery, [employeeName, email, contactNo, username, password], (err, result) => {
      if (err) {
...
// Add this route after your existing routes get all employee name
/*router.get('/getAllEmployeeNames', (req, res) => {
  ...
  db.query(selectQuery, (err, results) => {
    if (err) {
      ...
      console.log('Data retrieved from MySQL');
      // Extract only the employee names from the results
      const employeeNames = results.map(employee => employee.employeeName);
