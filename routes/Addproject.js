...
/*router.post('/addProject', upload.single('requirement'), (req, res) => {
  const {
    projectName,
    department,
    clientName,
  ...
  const insertQuery =
    'INSERT INTO project (projectName, department, clientName, mobileNo, email, startDate, deadlineDate, requirement, requirementNotes, Teamlead ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
  ...
  db.query(
    ...
    [
      ...
      startDate,
      deadlineDate,
      requirementFile.buffer, // Assuming the column type is BLOB
      requirementNotes,
      Teamlead,
    ],
    (err, result) => {
...
router.post('/addProjects', upload.single('requirement'), (req, res) => {
  ...
  db.query(
    ...
    [
      ...
      formattedDeadlineDate, // Use formatted date
      requirementFile.buffer, // Assuming the column type is BLOB
      requirementNotes,
      Teamlead,
      JSON.stringify(selectedMembers), // Convert array to JSON string
    ],
    (err, result) => {
      if (err) {
        console.error('Error inserting data into MySQL:', err);
